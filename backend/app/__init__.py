...
for module in Path(__file__).parent.glob("handlers/*.py"):
    sub_app.add_routes(importlib.import_module(f".handlers.{module.stem}", package=__package__).routes)
...
if STATIC_ROOT:
    ...
    app.middlewares.append(spa_middleware.with_html((STATIC_ROOT / "index.html").read_bytes()))
