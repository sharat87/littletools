...
export default class extends ToolView {
			...
			m(
				...
				[
					...
					m(".btn-group", [
						...
						}, "Download HTML file with this form"),
					]),
				],
			),
		]
	...
	makeFormHTML(): string {
		return [
			...
			`<form method="${ this.method() }" action="${ this.action() }" enctype="${ this.enctype() }">`,
			...this.fields.map((field: Field) => `<input type="${ field.isFile() ? "file" : "text" }" name="${ field.name }" value="${ field.value }">`),
			"</form>",
